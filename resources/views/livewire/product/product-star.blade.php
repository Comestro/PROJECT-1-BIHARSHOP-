...
<div class="">
    <div class="flex  items-center gap-2">
            ...
            <div class="flex items-center">
                @for ($i = 1; $i <= 5; $i++)
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="w-3 h-4 fill-current {{ $i <= $averageRating ? 'text-yellow-500' : 'text-gray-300' }}"
        ...
        <span class="text-gray-600 text-xs sm:text-sm flex truncate">| {{ $totalReviews }}
            {{ $totalReviews == 1 ? 'review' : 'reviews' }}</span>
    </div>
    <span class="text-gray-600 hidden md:flex">({{ $totalReviews }}
        {{ $totalReviews == 1 ? 'review' : 'reviews' }})</span>
</div>
