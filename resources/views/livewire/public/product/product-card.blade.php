<div class="w-full max-w-sm  rounded-lg ">
        <a wire:navigate href=" {{route('product.view',['category'=>$item->category->cat_slug, 'slug' => $item->slug])}}">
            <div class="rounded-2xl flex  bg-zinc-100 overflow-hidden">
                <img class="object-cover object-top h-[200px] lg:h-[450px] w-full rounded-t-lg" src="{{ $item->image ? asset('storage/image/product/' . $item->image) : asset('path/to/default-image.jpg') }}" alt="product image" />
            </div>
